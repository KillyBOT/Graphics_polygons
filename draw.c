...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  if(polygons->lastcol < 3){
    printf("Error! Need 1 or more triangles!\n");
  }
...
void add_box( struct matrix * edges,
  ...
  add_polygon(edges, x1, y0, z1, x1, y1, z0, x1, y1, z1);
  ...
  add_polygon(edges, x0, y0, z1, x0, y0, z0, x1, y0, z1);
...
void add_sphere( struct matrix * edges,
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    ...
    add_polygon(edges,
        ...
        points->m[0][index+1+steps], points->m[1][index+1+steps], points->m[2][index+1+steps]
    );

    add_polygon(edges,
        points->m[0][index+steps-2], points->m[1][index+steps-2], points->m[2][index+steps-2],
        points->m[0][index+steps-1], points->m[1][index+steps-1], points->m[2][index+steps-1],
        points->m[0][index+steps+steps-2], points->m[1][index+steps+steps-2], points->m[2][index+steps+steps-2]
    );
  ...
  /*for(int s = 0; s < steps; s++){
    semicircle = s * steps + 1;
    for(int point = 1; point <= steps-2; point++){
      add_polygon(
        edges, 
        points->m[0][point+semicircle], points->m[1][point+semicircle], points->m[2][point+semicircle], 
        points->m[0][point+1+semicircle], points->m[1][point+1+semicircle], points->m[2][point+1+semicircle],
        points->m[0][point+steps+1+semicircle], points->m[1][point+steps+1+semicircle], points->m[2][point+steps+1+semicircle]
      );
      add_polygon(
        edges, 
        points->m[0][point+1+steps+semicircle], points->m[1][point+1+steps+semicircle], points->m[2][point+1+steps+semicircle], 
        points->m[0][point+steps+semicircle], points->m[1][point+steps+semicircle], points->m[2][point+steps+semicircle],
        points->m[0][point+semicircle], points->m[1][point+semicircle], points->m[2][point+semicircle]
      );
    }
  }*/
